layui.define([
	...
	function initMapControl() {
		...
		map.on('draw:created', function(e) {
			...
			`;
			popup = L.popup().setContent(content).setLatLng(drawLayer._latlng).openOn(map)
			maputil.initMapEvent();
		});
	...
	function getTable(result, pageIndex, pageSize) {
		...
		if(result != "" && result != null) {
			...
			$.each(result.data, function(idx, item) {
				...
				let id = item.id;
				data += `<div class="site-item" data="${id}" onclick="zoomTo('${latitude}','${longitude}')">
										<div class="item-left">
											<span>${orderNum}</span>
										</div>
